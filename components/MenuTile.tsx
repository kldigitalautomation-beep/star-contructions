...
export function MenuTile({ icon, label, description, onPress, badge }: MenuTileProps) {
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.tile, pressed ? styles.pressed : undefined]}>
      <LinearGradient
        colors={theme.isDark ? ['rgba(122,170,255,0.18)', 'rgba(45,90,168,0.28)'] : ['rgba(22,58,124,0.12)', 'rgba(22,58,124,0.06)']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
...
function makeStyles(theme: AppTheme) { return StyleSheet.create({
  ...
  pressed: {
    opacity: theme.isDark ? 0.92 : 0.82,
  ...
  iconWrap: {
    ...
    flexShrink: 0,
    borderWidth: 1,
    borderColor: theme.isDark ? 'rgba(122,170,255,0.26)' : theme.colors.primaryMuted,
  },
  textWrap: {
    flex: 1,
    gap: 3,
  ...
  chevronWrap: {
    ...
    height: 30,
    borderRadius: 15,
    backgroundColor: theme.isDark ? theme.colors.accentSoft : 'rgba(245,166,35,0.12)',
    borderWidth: 1,
    borderColor: theme.isDark ? 'rgba(245,166,35,0.32)' : 'rgba(245,166,35,0.24)',
    alignItems: 'center',
    justifyContent: 'center',
    flexShrink: 0,
  },
